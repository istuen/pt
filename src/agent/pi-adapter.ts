...
import { refreshInjectionFooter } from "../manual-session.js";
      ...
      try {
        ...
        if (!currentSegment) {
          ...
          if (sessionState) {
            ...
            // v18.x（issue pt-footer-status-stale-cache）：改调 refreshInjectionFooter
            // ——拼 manual suffix + 同步 lastFooterText，避免 PiAdapter 直写 setStatus
            // 绕过缓存导致 manual suffix 丢失 / stale 缓存。
            if (api.ui) refreshInjectionFooter(api.ui, sessionState);
          }
          return undefined;
        }
        const event = args[0];
