...
### modname-2-forms
- desc: modName 2 形态（v9.1+）——(1) 段名（`Scene` / `User` / `Agent` / `Trigger` / `Rules` / `Flows` / `Checklists`）：跨所有引用域该段整段聚合；(2) 段.项（`Agent.role` / `User.who-am-i`：builtin 实际可用项）：跨所有引用域该段下 H3 项精确选。6 角色 agent-info 示例（`Agent.agent-role-senior-developer` 等）需 fullstack pack 或自建多角色 agent-info。**不引入形态 3（domain:段.项）**——限定到单 domain 通过专用段名（`User` / `Agent`）实现，避免 profile 重复写引用。

### section-as-namespace
- desc: 段名 = 命名空间（v9.1+）——通用段（Scene / Trigger / Rules / Flows / Checklists）跨 domain 通用；专用段（User（user-info 用）/ Agent（agent-info 用）/ ...）限定到单 domain。加新专用段 = Domain 用新 H2 段名 + `KNOWN_SECTION_NAMES` 集合加一行 + `moduleRenderers` 注册一行（详见 authoring 的 renderer-registration）。
...
### role-isolation
- desc: 角色隔离（v9.1+ 核心承诺）——通过专用段名 + 段.项粒度实现 Profile 间的角色隔离。builtin 示例：pt-dev profile 用 `[Scene, User, Agent.role, Agent.collaboration]`（只看 dev 需要的角色 + 协作定义）；pt-arch 用 `[Scene, User, Agent.role, Agent.boundaries]`（只看架构 + 边界）；pt-devops 用 `[Scene, User, Agent.collaboration, Agent.boundaries]`。**完整 4-profile 隔离（pt-arch / pt-design / pt-dev / pt-devops）+ 6 角色 agent-info 是 fullstack pack 或自建场景**——builtin agent-info 只有 3 个 H3 项（role / collaboration / boundaries），足以演示形态 2 粒度，但 4-profile 强隔离需要 `agent-role-architect` / `agent-role-senior-developer` 等细角色项（仅 fullstack 有）。**builtin 场景下所有 profile 共享同一 Blueprint（pt-default）+ 同一 Domain 集合（user-info + agent-info + ...），差异全在 ### Modules 段——真正的隔离**。

### h3-name-uniqueness
- desc: H3 项名唯一约束（v9.1+）——同段内 H3 重名 parse 时**后覆盖前**（保持 IR 唯一性）；跨段同名 H3 由段名命名空间避免（`Scene.x` 不匹配 `User` 段下同名 x，要跨段匹配必须改段名）。**`### <domain>.<item>` 渲染形式**——H3 项粒度产物输出形式（如 `### agent-info.role`——builtin 实际渲染；`### agent-info.senior-developer` 需 fullstack agent-info 才有）。
