...
### profile-format
- desc: frontmatter（name + blueprint: 引用名 + domains: [全局 Domain 列表，**多级目录用 path 形式**如 `workflow/dev-workflow`，自动分发到所有聚合组]）+ H2 聚合组实例化（## 聚合组名 与 Blueprint.groups[].name 同名）+ H3 段下两类追加列表：(a) `### Domains` 追加到本聚合组的 Domain 名列表；(b) `### Modules` 本插槽填的 modName 列表（v9.1+）。**v9.1+ 的 modules 形态 2 种**：段名（`Scene` / `User`）和 段.项（`Agent.role` builtin 实际可用；`Agent.agent-role-senior-developer` 等 6 角色场景需 fullstack pack 或自建 agent-info），无形态 3。domains 顺序影响 LLM attention——身份类放前，约束类放后。文件放 .pt/assets/profiles/<name>.profile.md。

### profile-modules-section
- desc: v9.1+ Profile H2 段下 ### Modules 段填法——按 modName 2 形态逐行列出本插槽的 modules。**形态 1：段名**（整段聚合，跨所有引用域）——例 `- Scene`（所有引用域的 Scene 段都进）。**形态 2：段.项**（精确选 H3 项）——例 `- Agent.role`（builtin agent-info 段下 role 项进；6 角色场景用 `- Agent.agent-role-senior-developer` 需 fullstack pack 或自建 agent-info）。modName 解析失败（段名不在 `KNOWN_SECTION_NAMES` 集合）→ log warn 跳过。**解析规则**：`Scene` / `User.who-am-i` / `Trigger.authoring-trigger` 都合法；`my-domain:Scene.foo` 这种形态 3 形式不合法——限定到单 domain 用专用段名（`User` / `Agent`）。

### profile-section-namespace
- desc: 段名 = 命名空间（v9.1+）——通用段（`Scene` / `Trigger` / `Rules` / `Flows` / `Checklists`）跨 domain 通用；专用段（`User` / `Agent`）限定到单 domain（`user-info` 用 `User`、`agent-info` 用 `Agent`）。**Profile 角色隔离的核心机制**——专用段名让 profile 可以"只选自己需要的身份角色"而不影响其他 profile。builtin `agent-info` 含 3 个 H3 项（`role` / `collaboration` / `boundaries`），pt-arch 与 pt-dev 用 `### Modules: [Agent.role, Agent.collaboration]` 等不同子集就能完成隔离；多角色 agent-info（4+ 角色项如 architect / senior-developer / qa-engineer / ...）是 **fullstack pack 或自建多角色 agent-info 场景**——见 profile-role-isolation 末尾的"进阶参考"块。

### profile-role-isolation
- desc: v9.1+ Profile 角色隔离配置示例（共享 agent-info Domain，各 profile 只看自己需要的 H3 项）。builtin `agent-info` 含 3 个 H3 项（`role` / `collaboration` / `boundaries`），下面 3 个 sample 演示 builtin 实际可用的隔离；**多角色 agent-info（4+ 角色项如 architect / senior-developer / ...）+ 多 Profile 隔离是 fullstack pack 或自建场景**——见末尾"进阶参考"块。
- sample-pt-arch: |
    ## session-context
    ### Modules
    - Scene
...
- advanced-reference: |
    > ⚠️ **进阶参考（需 fullstack pack 或自建多角色 agent-info）**
    >
    > builtin agent-info 只有 3 个 H3 项；外部 pack（如 fullstack）扩展到 6 个角色（`architect` / `senior-developer` / `qa-engineer` / `code-reviewer` / `devops-engineer` / `active-role-rule`），可演示更细的 4-profile 隔离：
    >
    > ```yaml
    > # sample-pt-arch（fullstack 版）
    > ## session-context
    > ### Modules
    > - Scene
    > - User
    ...
    > ```
    >
    > ⚠️ builtin agent-info **没有** `agent-role-*` / `active-role-rule` 这些 H3 项——照上面 fullstack sample 配 builtin agent-info 会静默无产物输出，**需要先扩展 agent-info 或装 fullstack pack**。
...
- desc: 身份类 Domain 写法（v9.1+ 重命名）——user-info Domain 用 `## User` 段下 H3 角色项（who-am-i / preferences / goals / ...）；agent-info Domain 用 `## Agent` 段下 H3 角色项（role / collaboration / boundaries / ...）。**段名 = 命名空间**：`User` 限定 user-info、`Agent` 限定 agent-info。Profile 通过 `### Modules: [User]` 聚合 user-info 整段；通过 `### Modules: [Agent.role]` 精确选单角色——角色隔离的"开关"。多角色 agent-info（如 `Agent.agent-role-senior-developer`）需 fullstack pack 或自建扩展。
...
- step: 角色隔离配置——身份域用专用段名 + 段.项精确选单角色（详见 profile-role-isolation 的 builtin sample + 进阶参考块）
...
- items: [4 profile（pt-arch / pt-design / pt-dev / pt-devops）共用同一 agent-info Domain 但各自 ### Modules 列出不同 Agent 角色 —— **builtin 场景只有 3 个 H3 项（role / collaboration / boundaries）；4+ profile + 6 角色 agent-info 隔离是 fullstack pack 或自建场景**（见 profile-role-isolation 进阶参考块）, pt-dev `### Modules` 含 `Agent.agent-role-senior-developer` + `Agent.agent-role-qa-engineer` + `Agent.agent-role-code-reviewer` + `Agent.active-role-rule`（不含 architect / devops-engineer）—— **仅 fullstack 场景；builtin agent-info 无 agent-role-* / active-role-rule 项**, pt-arch / pt-design 含 `Agent.agent-role-architect` + `Agent.active-role-rule`（不含 dev / devops 角色）—— **仅 fullstack 场景**, pt-devops 含 `Agent.agent-role-devops-engineer` + `Agent.active-role-rule`（不含其他角色）—— **仅 fullstack 场景**, 跑 transpile 后 /pt raw 检查产物——`### agent-info.<item>` H3 粒度输出仅在应见 profile 中出现；builtin 场景看 `### agent-info.role` / `### agent-info.collaboration` / `### agent-info.boundaries` 是否按 ### Modules 选择出现]