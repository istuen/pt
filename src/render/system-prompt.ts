// src/render/system-prompt.ts — AgentContext 注入点 → System Prompt 字符串
//
...
//   函数名 renderSystemPrompt 不改（P4 会改→renderSessionPrompt）。

import type { AgentContext, Blueprint } from "../schema.js";

/** AgentContext 注入点聚合 → System Prompt 字符串。
 *  v9：遍历 Blueprint.injectionPoints，聚合所有 target=system_prompt 的注入点内容。 */
export function renderSystemPrompt(ctx: AgentContext, blueprint: Blueprint): string {
  const parts: string[] = [];
