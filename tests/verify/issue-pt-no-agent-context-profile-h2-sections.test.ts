...
describe("parseProfile groups (v13.x issue pt-no-agent-context-profile-h2-sections)", () => {
  ...
  // 项目 .pt/assets/profiles/ 为空，profile 从 fullstack pack 加载。
  const fullstackProfilesDir = path.join(process.cwd(), ".pt/packs/fullstack/profiles");
  const builtinProfilesDir = path.join(process.cwd(), "src/builtin/assets/profiles");
