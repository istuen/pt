...
import type { AgentContext, CompilationConfig } from "../../src/schema.js";
...
describe("saveAgentContext + loadAgentContext round-trip", () => {
  it("save → load 同 sourceHash 命中", async () => {
    ...
    await saveAgentContext(tmpDir, ctx, COMPILATION);
    const loaded = await loadAgentContext(tmpDir, ctx.name, ctx.sourceHash, COMPILATION);
  ...
  it("hash mismatch 降级 → 返 null", async () => {
    ...
    await saveAgentContext(tmpDir, ctx, COMPILATION);
    const loaded = await loadAgentContext(tmpDir, ctx.name, "different-hash-00000000", COMPILATION);
    expect(loaded).toBeNull();
  });

  it("文件不存在 → 返 null（首次加载）", async () => {
    const loaded = await loadAgentContext(tmpDir, "nonexistent", "any-hash", COMPILATION);
    expect(loaded).toBeNull();
  });
  ...
  it("文件损坏（无 frontmatter）→ 返 null", async () => {
    ...
    const loaded = await loadAgentContext(tmpDir, "test-ctx", "any-hash", COMPILATION);
    expect(loaded).toBeNull();
  });
  ...
  it("save 写入路径包含 cacheDir", async () => {
    const ctx = makeContext();
    const file = await saveAgentContext(tmpDir, ctx, COMPILATION);
